...
namespace posets::utils {
  ...
  class sharingtree {
    private:
      ...
      // The hash function is based on the label of the node and the indices
      // of its children.
      class st_hash {
          sharingtree* f;

        public:
          st_hash (sharingtree* that) : f {that} {}

          size_t operator() (const st_node& k) const {
            size_t res = std::hash<typename V::value_type> () (k.label);
            int child_idx = k.left;
            int i = 1;
            while (child_idx >= 0) {
              res ^= std::hash<int> () (child_idx) << i++;
              st_node* child = f->bin_tree[child_idx];
              child_idx = child->left;
            }
            return res;
          }
      };

      // TODO: continue making it sharingtree from here
      class st_equal {
          sharingforest* f;
